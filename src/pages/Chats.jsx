...
export default function Chats() {
  ...
  const [receiverUsername, setReceiverUsername] = useState(''); // ← NOW USERNAME
  ...
  useEffect(() => {
    ...
    pb.collection('messages').subscribe('*', (e) => {
      if (e.action === 'create') {
        pb.collection('messages').getOne(e.record.id, { expand: 'sender,receiver' }).then(msg => {
  ...
  return (
    <div style={{ padding: '20px', paddingBottom: '100px' }}>
      ...
      <input
        type="text"
        placeholder="Receiver username (e.g. john123)"
          ...
          >
            ...
            >
              <p style={{ margin: 0, fontSize: '14px', fontWeight: 'bold' }}>
                {m.expand?.sender?.username || 'You'}
              </p>
              <p style={{ margin: '4px 0 0', fontSize: '14px' }}>{m.content}</p>
